...
export function startServer(options: ServerOptions = {}) {
  const port = options.port ?? parseInt(process.env.PORT ?? process.env.PRODUCER_PORT ?? "9847", 10);
  const log = options.logger ?? defaultLogger;
...
if (isPublicServerEntry) {
  const { values } = parseArgs({
    options: {
      port: { type: "string", short: "p", default: process.env.PORT ?? process.env.PRODUCER_PORT ?? "9847" },
    },
  });
